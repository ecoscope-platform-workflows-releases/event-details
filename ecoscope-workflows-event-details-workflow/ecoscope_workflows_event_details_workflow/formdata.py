...
class SetEventDetailsCombined(BaseModel):
    model_config = ConfigDict(
        extra="forbid",
    )
    event_type: str = Field(..., title="Events Type")
    analysis_field: str = Field(
        ...,
    ...
    category_field_label: Optional[str] = Field(
        "",
...
class EventsBarChart(BaseModel):
    events_bar_chart: Optional[EventsBarChart1] = Field(None, title="")
...
class AnalysisFieldSumMap(BaseModel):
...
class FormData(BaseModel):
    ...
    set_event_details_combined: Optional[SetEventDetailsCombined] = Field(
        None, title="Event Types"
    )
    ...
    Analysis_Field_Sum_Map: Optional[AnalysisFieldSumMap] = Field(
        None,
        alias="Analysis Field Sum Map",
